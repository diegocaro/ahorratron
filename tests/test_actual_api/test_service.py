...
from actual_api.models import Transaction
from actual_api.service import ActualBudgetService
...
def test_add_transaction_with_autospec(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("actual_api.service.Actual", mock_actual)

    mock_create_transaction = MagicMock()
    mock_create_transaction.return_value.id = "1234"
    monkeypatch.setattr(
        "actual_api.service.create_transaction", mock_create_transaction
    )

    mock_get_account = MagicMock()
    monkeypatch.setattr("actual_api.service.get_account", mock_get_account)
